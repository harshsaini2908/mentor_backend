...
// Route to fetch students with a specific mentor ID
app.get('/students/with-mentor/:mentorId', async (req, res) => {
  const { mentorId } = req.params;
  try {
    const query = 'SELECT * FROM students WHERE mentor_id = $1';
    const { rows } = await pool.query(query, [mentorId]);
    res.json(rows);
  } catch (error) {
    console.error('Error fetching students with mentor ID:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.listen(port, () => {
  console.log(`Server is running on http://harsh-db-harshsaini2908-001.a.aivencloud.com:${port}`);
});
